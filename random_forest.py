...
roc_auc = auc(false_positive_rate, true_positive_rate)
roc_auc = round(roc_auc, 4)
print("RF Model's ROC AUC value : ", roc_auc)
...
plt.savefig((base_path/"../static/images/ROC_Curve_RF.png").resolve(), dpi=100, pad_inches=0.5, bbox_inches="tight")
plt.clf()

# Saving confusion matrix
sns.heatmap(cnf_matrix/np.sum(cnf_matrix), annot=True, fmt='.2%', cmap='Blues').set_title("RF Confusion Matrix").figure.savefig((base_path/"../static/images/Confusio_Matrix_RF.png").resolve(), dpi=100)

# Saving ML model and result
pickle.dump(rf_imp, open((base_path/"../pickle_global/rf_model.pkl").resolve(),'wb'))
pickle.dump(selected_feat, open((base_path/"../pickle_global/rf_selected_feat.pkl").resolve(),'wb'))
pickle.dump(roc_auc, open((base_path/"../pickle_global/rf_roc_auc.pkl").resolve(),'wb'))
pickle.dump(cnf_matrix, open((base_path/"../pickle_global/rf_cnf_matrix.pkl").resolve(),'wb'))
pickle.dump(report, open((base_path/"../pickle_global/rf_report.pkl").resolve(),'wb'))
pickle.dump(accuracy, open((base_path/"../pickle_global/rf_accuracy.pkl").resolve(),'wb'))